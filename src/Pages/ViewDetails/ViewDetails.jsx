...
const ViewDetails = () => {
  ...
  const relatedCar = products.filter(car=>car?.category === category && car._id !== id);
  console.log(relatedCar);
  return (
    ...
    <div className="grid grid-cols-2 gap-4 my-12  md:grid-cols-3 lg:grid-cols-4">
      {
      relatedCar?.map(product=><ProductCard product={product} key={product._id}></ProductCard>)
    }
    </div>
